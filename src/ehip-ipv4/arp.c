        ...
        switch (arp_table_entry->state){
            ...
            case ARP_STATE_NUD_STALE:
                if(arp_table_entry->delay_probe_time_cd){
                    ...
                    arp_table_entry->state = ARP_STATE_NUD_DELAY;
                }
                break;
            ...
            case ARP_STATE_NUD_PROBE:
                if(arp_table_entry->retry_cnt){
                    ...
                    if(ret < 0)
                        arp_table_entry->state = ARP_STATE_NUD_FAILED;
                }
            ...
            case ARP_STATE_NUD_DELAY:
                ...
                if(arp_table_entry->delay_probe_time_cd == 0 || --arp_table_entry->delay_probe_time_cd == 0 ){
                    ...
                    if(ret < 0)
                        arp_table_entry->state = ARP_STATE_NUD_FAILED;
                    arp_table_entry->state = ARP_STATE_NUD_PROBE;
                }
                break;
            case ARP_STATE_NUD_REACHABLE:
                ...
                if(arp_table_entry->reachable_time_cd == 0 || --arp_table_entry->delay_probe_time_cd == 0)
                    arp_table_entry->state = arp_table_entry->delay_probe_time_cd ? ARP_STATE_NUD_DELAY : ARP_STATE_NUD_STALE;

                break;
        }
        /* 对比新旧两种状态，看是否需要触发 signal_arptable_changed */
        if(arp_state_is_change(old_state, arp_table_entry->state)){
...
static bool arp_state_update_change_notify(int index, const ehip_hw_addr_t *new_hw_addr, uint8_t new_state){
    bool is_change;
    is_change = arp_state_is_change(arp_table[index].state, new_state);
    arp_table[index].state = new_state;

    if(is_change == false){
        for(int i=0; i<arp_table[index].netdev->attr.hw_addr_len; i++){
            if(((uint8_t*)new_hw_addr)[i] != ((uint8_t*)&arp_table[index].hw_addr)[i]){
                is_change = true;
                break;
            }
        }
    }
    memcpy(arp_table[index].hw_addr.addr, new_hw_addr, arp_table[index].netdev->attr.hw_addr_len);
    if(is_change)
        eh_signal_notify(&signal_arptable_changed);
    return is_change;
}
...
static int arp_find_entry(ipv4_addr_t ip_addr, ehip_netdev_t *netdev, bool is_create){
    size_t i;
    for( i=0;i<EHIP_ARP_CACHE_MAX_NUM;i++){
        if(arp_table[i].state == ARP_STATE_NUD_FAILED)
            continue;
        if(arp_table[i].ip_addr == ip_addr && arp_table[i].netdev == netdev)
            return (int)i;
    }
    /* TODO */
    (void) is_create;
...
static void arp_handle(struct ehip_buffer* buf){
    ...
    if( arp_hdr->ar_op == eh_hton16(ARPOP_REQUEST) && 
        ...
        if(s_ipv4_addr){
            arp_entry_idx = arp_find_entry(s_ipv4_addr, buf->netdev, true);
            if(arp_entry_idx >= 0)
    ...
    if( arp_hdr->ar_op == eh_hton16(ARPOP_REPLY) &&
        ...
        arp_entry_idx = arp_find_entry(s_ipv4_addr, buf->netdev, false);
        if(arp_entry_idx < 0) goto drop;
        arp_state_update_change_notify(arp_entry_idx, s_hw_addr, ARP_STATE_NUD_INCOMPLETE);
    }
