...
void icmp_error_input(struct ip_message *ip_msg, const struct icmp_hdr *icmp_hdr){
    ...
    eh_mdebugln( ICMP_ERROR_INPUT, "error sender: %d.%d.%d.%d", 
    ...
    eh_mdebugln( ICMP_ERROR_INPUT, "######## ERROR RAW IP PACKET ########");
    eh_mdebugln( ICMP_ERROR_INPUT, "src: %d.%d.%d.%d", 
    ...
    eh_mdebugln( ICMP_ERROR_INPUT, "dst: %d.%d.%d.%d", 
    ...
    eh_mdebugln( ICMP_ERROR_INPUT, "tos:%02x", err_ip_hdr->tos);
    eh_mdebugln( ICMP_ERROR_INPUT, "iphdr_len:%d", eh_ntoh16(err_ip_hdr->tot_len));
    eh_mdebugln( ICMP_ERROR_INPUT, "id:%d", eh_ntoh16(err_ip_hdr->id));
    eh_mdebugln( ICMP_ERROR_INPUT, "frag_off:%04x", eh_ntoh16(err_ip_hdr->frag_off));
    eh_mdebugln( ICMP_ERROR_INPUT, "ttl:%d", err_ip_hdr->ttl);
    eh_mdebugln( ICMP_ERROR_INPUT, "protocol:%02x", err_ip_hdr->protocol);
    eh_mdebugln( ICMP_ERROR_INPUT, "payload:|%.*hhq|", icmp_error_payload_len, icmp_error_payload);
    ...
    switch (err_ip_hdr->protocol) {
        case IP_PROTO_ICMP:{
            extern void ping_error_input(ipv4_addr_t err_sender, struct ip_hdr *ip_hdr, const uint8_t *payload, int payload_len, int error);
            ping_error_input(ip_msg->ip_hdr.src_addr, err_ip_hdr, icmp_error_payload, icmp_error_payload_len, error);
            break;
        }
        case IP_PROTO_UDP:
